...
- **持久化文件模型** — YOLO 标注格式（txt）、SAM2 掩码格式（png）
...
**关联关系：** 被 Predict Task、Training Task 引用作为输入。
...
1. **上传 → Image Record** — 图片元数据入库
2. **预测 → Predict Task → Result** — 推理结果返回前端
